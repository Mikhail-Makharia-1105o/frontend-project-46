...
export default function flatOutput(
  ...
  keys.forEach((key) => {
    if (comparisonObj[key].nested) {
      ...
      output.push(`Property '${currentPath}${currentPath ? '.' : ''}${key}' was added with value: ${typeof comparisonObj[key].value === 'object' && comparisonObj[key].value ? '[complex value]' : typeof comparisonObj[key].value === 'string' ? `'${comparisonObj[key].value}'` : comparisonObj[key].value}`);
    } else if (comparisonObj[key].type === 'removed') {
      output.push(`Property '${currentPath}${currentPath ? '.' : ''}${key}' was removed`);
    } else if (comparisonObj[key].type === 'changed') {
